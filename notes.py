...
class Notebook:
    ...
    def sort_notes_by_tags(self, tag):
        ...
        sorted_notes = sorted(notes_with_tag, key=lambda x: x.get('created_at', datetime.min), reverse=True)
        self.notes = sorted_notes + [note for note in self.notes if note not in notes_with_tag]
        return True
    ...
    def show_unique_tags(self):
        ...
        for note in self.notes:
            for tag in note['tags']:
                unique_tags.add(tag)
        print("Dostępne tagi:", ", ".join(unique_tags) if unique_tags else "Brak tagów")
